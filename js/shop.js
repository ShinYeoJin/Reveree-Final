const productLists = [
  {
    name: "STAIN REMOVER",
    price: "36,000원",
    details: "얼룩제거제 295ML",
    imgSrc: "/image/shop/products/stain_remover.jpg",
    categoryType: ["ALL", "REMOVER"],
    stock: "sold-out",
  },
  {
    name: "DETERGENT - FOREST",
    price: "45,000원",
    details: "세탁세제 900ML",
    imgSrc: "/image/shop/products/detergent_forest.jpg",
    categoryType: ["ALL", "DETERGENT"],
    stock: "available",
  },
  {
    name: "DETERGENT - WIND",
    price: "45,000원",
    details: "세탁세제 900ML",
    imgSrc: "/image/shop/products/detergent_wind.jpg",
    categoryType: ["ALL", "DETERGENT"],
    stock: "available",
  },
  {
    name: "[GIFT] DETERGENT",
    price: "50,000원",
    details: "선물포장 세탁세제 900ML",
    imgSrc: "/image/shop/products/detergent_gift.jpg",
    categoryType: ["ALL", "DETERGENT", "GIFT"],
    stock: "available",
  },
  {
    name: "[GIFT] STAIN REMOVER",
    price: "41,000원",
    details: "선물포장 얼룩제거제 295ML",
    imgSrc: "/image/shop/products/stain_remover_gift.jpg",
    categoryType: ["ALL", "REMOVER", "GIFT"],
    stock: "sold-out",
  },
  {
    name: "WASHING DUO",
    price: "95,000원",
    details: "2종 선물세트",
    imgSrc: "/image/shop/products/washing_duo.jpg",
    categoryType: ["ALL", "GIFT"],
    stock: "available",
  },
  {
    name: "ESSENTIAL DUO",
    price: "86,000원",
    details: "2종 선물세트",
    imgSrc: "/image/shop/products/essential_duo.jpg",
    categoryType: ["ALL", "GIFT"],
    stock: "available",
  },
  {
    name: "ULTIMATE TRIO",
    price: "131,000원",
    details: "3종 선물세트",
    imgSrc: "/image/shop/products/ultimate_trio.jpg",
    categoryType: ["ALL", "GIFT"],
    stock: "available",
  },
  {
    name: "[TRAVEL] REMOVER STICKER POUCH",
    price: "19,000원",
    details: "여행용 얼룩제거제 스티커 파우치",
    imgSrc: "/image/shop/products/remover_sticker_pouch_travel.jpg",
    categoryType: ["ALL", "REMOVER", "GIFT"],
    stock: "sold-out",
  },
  {
    name: "[TRAVEL] ESSENTIAL DUO",
    price: "33,000원",
    details: "여행용 2종 선물세트",
    imgSrc: "/image/shop/products/essential_duo_travel.jpg",
    categoryType: ["ALL", "GIFT"],
    stock: "sold-out",
  },
  {
    name: "[TRAVEL] STAIN REMOVER",
    price: "15,000원",
    details: "얼룩제거제 59ML",
    imgSrc: "/image/shop/products/stain_remover_travel.jpg",
    categoryType: ["ALL", "REMOVER"],
    stock: "sold-out",
  },
  {
    name: "[TRAVEL] DETERGENT - FOREST",
    price: "14,000원",
    details: "세탁세제 59ML",
    imgSrc: "/image/shop/products/detergent_forest_travel.jpg",
    categoryType: ["ALL", "DETERGENT"],
    stock: "sold-out",
  },
  {
    name: "[TRAVEL] DETERGENT - WIND",
    price: "14,000원",
    details: "세탁세제 59ML",
    imgSrc: "/image/shop/products/detergent_wind_travel.jpg",
    categoryType: ["ALL", "DETERGENT"],
    stock: "sold-out",
  },
  {
    name: "ECO BAG",
    price: "12,900원",
    details: "에코백",
    imgSrc: "/image/shop/products/eco_bag.jpg",
    categoryType: ["ALL", "ACC"],
    stock: "available",
  },
  {
    name: "LAVEREE X BLOOMINGTALE",
    price: "29,000원",
    details: "미니뿌 키링",
    imgSrc: "/image/shop/products/Laveree_bloomingtale.jpg",
    categoryType: ["ALL", "ACC"],
    stock: "available",
  },
  {
    name: "FELT BASKET",
    price: "9,900원",
    details: "펠트 바스켓",
    imgSrc: "/image/shop/products/felt_basket.png",

    categoryType: ["ALL", "ACC"],
    stock: "available",
  },
  {
    name: "SCRUNCHINE",
    price: "15,900원",
    details: "헤어 스크런치",
    imgSrc: "/image/shop/products/scrunchie.png",

    categoryType: ["ALL", "ACC"],
    stock: "available",
  },
  {
    name: "PUMP",
    price: "3,000원",
    details: "세제용 펌프",
    imgSrc: "/image/shop/products/pump.jpg",
    categoryType: ["ALL", "ACC"],
    stock: "sold-out",
  },
  {
    name: "GUN SPRAY",
    price: "1,000원",
    details: "건 스프레이",
    imgSrc: "/image/shop/products/gun_spray.jpg",
    categoryType: ["ALL", "ACC"],
    stock: "available",
  },
  {
    name: "MEASURING CUP",
    price: "1,000원",
    details: "세제용 계량컵",
    imgSrc: "/image/shop/products/measuring_cup.jpg",
    categoryType: ["ALL", "ACC"],
    stock: "available",
  },
  {
    name: "SHOPPING BAG",
    price: "1,000원",
    details: "쇼핑백 (XS)",
    imgSrc: "/image/shop/products/shopping_bag(XS).png",

    categoryType: ["ALL", "ACC"],
    stock: "available",
  },
  {
    name: "SHOPPING BAG",
    price: "1,500원",
    details: "쇼핑백 (S)",
    imgSrc: "/image/shop/products/shopping_bag(S).jpg",
    categoryType: ["ALL", "ACC"],
    stock: "available",
  },
  {
    name: "SHOPPING BAG",
    price: "2,000원",
    details: "쇼핑백 (M)",
    imgSrc: "/image/shop/products/shopping_bag(M).png",
    categoryType: ["ALL", "ACC"],
    stock: "available",
  },
];

const kf = [{ opacity: 0 }, { opacity: 1 }];
const dur = {
  duration: 600,
  fill: "forwards",
  easing: "ease-out",
};

export const initShop = (category) => {
  const grid = document.querySelector("#productsGrid");

  // 모바일 화면일 경우 topImage 변경
  const mediaSize = window.innerWidth;
  const topImage = document.querySelector("#photoThis");
  topImage.animate(kf, dur);

  if (mediaSize <= 768) {
    switch (category) {
      case "ALL":
        topImage.src = "/image/shop/topImages/shop_all_mobile.jpg";
        break;
      case "REMOVER":
        topImage.src = "/image/shop/topImages/shop_remover_mobile.jpg";
        break;
      case "GIFT":
        topImage.src = "/image/shop/topImages/shop_gift_mobile.jpg";
        break;
    }
  }

  productLists
    .filter((v) => v.categoryType.includes(category))
    .forEach((v) => {
      // ul(상품 매대 그리드)태그, 안에 li 로 물건 정렬 할 것.

      // grid(ul) 내부
      const prdLis = document.createElement("li");
      prdLis.classList.add("prdLis", ...v.categoryType);
      // ul 자식, 아래 요소 부모
      const prdLisPic = document.createElement("picture");
      prdLisPic.id = "prdLisPic";
      // 물건 픽쳐, prdLis 자식
      const prdLisPicImg = document.createElement("img");
      prdLisPicImg.id = "prdLisPicImg";
      // 물건 이미지, prdLisPic 자식, v.imgSrc 삽입
      const prdLisDesc = document.createElement("div");
      prdLisDesc.id = "prdLisDesc";
      // 물건 설명, prdLis 자식
      const prdLisDescDetail = document.createElement("div");
      prdLisDescDetail.id = "prdLisDescDetail";
      // 이름 및 가격, space-between 사용, prdLisDesc 자식
      const prdLisDescDetailName = document.createElement("span");
      prdLisDescDetailName.id = "prdLisDescDetailName";
      // 이름, prdLisDescDetail 자식, v.name 삽입
      const prdLisDescDetailPrice = document.createElement("span");
      prdLisDescDetailPrice.id = "prdLisDescDetailPrice";
      // 가격, prdLisDescDetail 자식, v.price 삽입
      const prdLisDescDetailUse = document.createElement("span");
      prdLisDescDetailUse.id = "prdLisDescDetailUse";
      // 최하위 설명, prdLisDesc 자식, v.details 삽입

      const prdLisStock = document.createElement("div");
      prdLisStock.id = "prdLisStock";
      // 품절 이미지 div, prdLis 자식
      const prdLisStockSoldout = document.createElement("img");
      // 품절 이미지, prdLisStock 자식

      // 품절 여부 분류
      v.stock == "sold-out"
        ? (prdLisStockSoldout.src = "/image/shop/icons/productIcon_soldout.gif")
        : null;

      (prdLisDescDetailName.innerText = v.name),
        (prdLisDescDetailPrice.innerText = v.price),
        (prdLisDescDetailUse.innerText = v.details),
        (prdLisPicImg.src = v.imgSrc);

      grid.appendChild(prdLis);
      prdLis.append(prdLisPic, prdLisDesc, prdLisStock);
      prdLisStock.appendChild(prdLisStockSoldout);
      prdLisPic.appendChild(prdLisPicImg);
      prdLisDesc.append(prdLisDescDetail, prdLisDescDetailUse);
      prdLisDescDetail.append(prdLisDescDetailName, prdLisDescDetailPrice);
      prdLis.animate(kf, dur);
    });
};
